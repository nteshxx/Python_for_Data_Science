...
def categorical(df):
    ...
    categorical_var = list(categorical_var.columns.values)
...
def numerical(df):
    ...
    numerical_var = list(numerical_var.columns.values)
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    df[date_col] = pd.to_datetime(df[date_col],format = "%Y-%m-%d %H:%M:%S").dt.strftime('%B')
    aggregated_value = pd.pivot_table(df, values=agg_col, index=date_col, aggfunc=agg)
...
def group_values(df,col1,agg1):
   ...
   Keyword arguments:
    ...
    """
    grouping = df.groupby(df[col1]).agg(agg1)
    return grouping
...
count = clear(df,col='Weather',val='Cloudy')
print(count)
...
wind_speed_35_vis_25 = instances_based_condition(df,col1='Wind Spd (km/h)',val1=35,col2='Visibility (km)',val2=25)
print(wind_speed_35_vis_25.head())
print(len(wind_speed_35_vis_25))
...
z = agg_values_ina_month(df,date_col='Date/Time',agg_col='Temp (C)',agg=['mean','max', 'min', 'sum'])
print(z)
...
mean_weather = group_values(df,'Weather',['mean'])
